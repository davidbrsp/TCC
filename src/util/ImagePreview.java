...
class ImagePreview extends JComponent implements PropertyChangeListener {
	...
	/**
	 * Register a property change listener with the file chooser so that the
	 * ImagePreview component is made aware of file chooser events (such as
	 * a user selecting a file).
	 */
	...
	this.iconPanel.setPreferredSize(new Dimension(WIDTH, HEIGHT));
	 ...
	 * thumbnail to disappear when the user selects a directory, for
	 * example.
	 */
	if (icon != null) {
	    /**
	    ...
	    Rectangle bounds = new Rectangle(iconPanel.getX(),
		    iconPanel.getY(), icon.getIconWidth(), icon.getIconHeight());
	     ...
	     */
	    icon.paintIcon(iconPanel, g, iconPanel.getX(), iconPanel.getY());
	    // iconPanel.paint(g);
	}
    }
	...
	if (JFileChooser.SELECTED_FILE_CHANGED_PROPERTY.equals(propName)) {
	     ...
	     * previous ImageIcon() constructor will execute, but the image will
	     * not be read from the file -- it will be read from the cache (with
	     * -1 as the width and as the height). No image will appear in the
	     * preview window; the user will be confused. The solution to this
	     * problem is to test the Image's width for -1. If this value is
	     * present, Image's flush() method is called on the Image, and a new
	     * ImageIcon is created. Internally, Toolkit's getImage() method
	     ...
	     */
	    // DAVID
	    int maxWH = Math.max(icon.getIconWidth(), icon.getIconHeight());

	    iconPanel.setLocation(0, 0);
	    // escala
	    if (icon.getIconWidth() == maxWH)
		...
		// this.getWidth(), -1, Image.SCALE_DEFAULT));
	    ...
	    // centraliza
	    if (this.getHeight() > icon.getIconHeight() + 1)
		iconPanel.setLocation(iconPanel.getX(), iconPanel.getY()
			+ icon.getIconHeight() / 2);
	    if (this.getWidth() > icon.getIconWidth() + 1)
		iconPanel.setLocation(iconPanel.getX() + icon.getIconWidth()
			/ 2, iconPanel.getY());
	    // icon.paintIcon(iconPanel, g, x, y)
	    this.add(iconPanel);
	    // Display image.
	    repaint();
	}
